...
public class AdminThemSanPhamController {
    ...
    public String themSanPham(@RequestParam("tenSanPham") String tenSanPham,
        ...
        if (nguoiDung == null) {
            redirectAttributes.addFlashAttribute("errorMessage", "Bạn cần đăng nhập để thêm sản phẩm.");
            return "redirect:/dangnhap";
        }

        if (hinhAnhFile.isEmpty()) {
            addFormData(model);
            model.addAttribute("errorMessage", "Vui lòng chọn hình ảnh sản phẩm.");
            return "adminTemplate/themsanpham";
        }

        String fileName = StringUtils.cleanPath(
                hinhAnhFile.getOriginalFilename() == null ? "san-pham.jpg" : hinhAnhFile.getOriginalFilename());
        if (!hinhAnhFile.isEmpty()) {
            try {
        ...
        try {
            switch (loaiSPID) {
                ...
                default:
                    ...
                    addFormData(model);
                    model.addAttribute("errorMessage", "Loại sản phẩm không hợp lệ.");
                    return "adminTemplate/themsanpham";
            }
        } catch (Exception e) {
            sanPhamService.xoaSanPham(sanPham);
